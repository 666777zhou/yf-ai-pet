...
BoxAudioCodec::BoxAudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    ESP_LOGI(TAG, "BoxAudioCodec initialized (ES8311+ES7210)");
}
...
void BoxAudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws,
    ...
    // TX: standard I2S for speaker output
...
#if SOC_I2S_SUPPORTS_TDM
    // RX: TDM mode for ES7210 4-channel mic input
    auto slot_mask = static_cast<i2s_tdm_slot_mask_t>(
    ...
    ESP_LOGI(TAG, "Duplex channels created (TX=std, RX=tdm)");
#else
    // Fallback: init RX as standard mode if TDM is not supported
    ...
    ESP_LOGI(TAG, "Duplex channels created (TX=std, RX=std fallback)");
#endif
}
...
void BoxAudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        ESP_ERROR_CHECK(esp_codec_dev_set_in_channel_gain(
            input_dev_, ESP_CODEC_DEV_MAKE_CHANNEL_MASK(0), AUDIO_CODEC_DEFAULT_MIC_GAIN));
    } else {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    }
